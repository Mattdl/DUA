...
from train.LwF.ModelWrapperLwF import *
import warnings
...
def train_model_lwf(model, original_model, criterion, optimizer, lr, dset_loaders, dset_sizes, use_gpu, num_epochs,
                    exp_dir='./', resume='', temperature=2, model_epoch_saving_freq=5, reg_lambda=1):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if epoch % model_epoch_saving_freq == 0:
...
def fine_tune_SGD_LwF(manager, init_model_path='', num_epochs=100, lr=0.0004, init_freeze=1, weight_decay=0,
                      model_epoch_saving_freq=5, reg_lambda=1):
    print('lr is ' + str(lr))
    ...
    if os.path.isfile(resume):
        ...
        previous_model = torch.load(manager.previous_task_model_path)
        if not (type(previous_model) is ModelWrapperLwF):
            last_layer_index = (len(model_ft.classifier._modules) - 1)
            previous_model = ModelWrapperLwF(previous_model, last_layer_name=last_layer_index)
        original_model = copy.deepcopy(previous_model)
        del checkpoint
        del previous_model
    else:
        if not os.path.isfile(manager.previous_task_model_path):
        ...
        if not (type(model_ft) is ModelWrapperLwF):
            last_layer_index = (len(model_ft.classifier._modules) - 1)
            model_ft = ModelWrapperLwF(model_ft, last_layer_name=last_layer_index)
        ...
        if not init_freeze:
            model_ft.model.classifier.add_module(str(len(model_ft.model.classifier._modules)),
                                                 nn.Linear(model_ft.num_ftrs, len(manager.dset_classes)))
        else:
            init_model = torch.load(init_model_path)
            last_layer_index = (len(model_ft.classifier._modules) - 1)
            model_ft.model.classifier.add_module(str(len(model_ft.model.classifier._modules)),
                                                 init_model.classifier[last_layer_index])
            del init_model
    ...
    # Reset BN params
    utils.reset_BN_layers(model_ft.model)

    criterion = nn.CrossEntropyLoss()
    optimizer_ft = optim.SGD(model_ft.parameters(), lr, momentum=0.9, weight_decay=weight_decay)
    model_ft = train_model_lwf(model_ft, original_model, criterion, optimizer_ft, lr, manager.dset_loaders,
                               manager.dset_sizes, use_gpu, num_epochs, manager.exp_dir, resume,
                               model_epoch_saving_freq=model_epoch_saving_freq, reg_lambda=reg_lambda)
