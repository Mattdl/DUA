...
class Objective_After_SGD(optim.SGD):
    ...
    def __init__(self, params, lr=0.001, momentum=0, dampening=0,
                 weight_decay=0, nesterov=False, cuda=True):
        self.cuda = cuda
...
def train_model(model, criterion, optimizer, lr, dset_loaders, dset_sizes, use_gpu, num_epochs, exp_dir='./',
                resume='', model_epoch_saving_freq=5):
...
def compute_importance_l2(model, optimizer, lr_scheduler, dset_loaders, use_gpu, mode=None):
    print("COMPUTING L2 IWS: use_gpu={}, mode={}".format(use_gpu, mode))
    since = time.time()
    ...
    for dset_loader in dset_loaders:
        for data in dset_loader:
            ...
            if mode == 'ground_truth_only':
                ...
                gt_output = outputs[torch.arange(outputs.size()[0]), labels]
                # print("GT_OUTPUT SHAPE = {}".format(gt_output.shape))
                # print(gt_output)
                # print("gt_output[0] = {}".format(gt_output[0]))
                target_zeros = torch.zeros_like(gt_output)
                outputs = gt_output
            else:
                # compute the L2 norm of output
                target_zeros = torch.zeros(outputs.size())

                if mode == 'afterSoftmax':
            ...
            mean_output = mean_output + targets if mean_output is not None else targets
            targets.backward()
            del targets
            # print('step')
    ...
    mean_output /= index
    torch.cuda.empty_cache()
    print("=> MEAN L2 OUTPUT = {}".format(mean_output))
    return model
