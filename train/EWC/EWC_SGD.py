...
class Weight_Regularized_SGD(optim.SGD):
    ...
    def step(self, reg_params, closure=None):
        ...
        if closure is not None:
            loss = closure()
        index = 0
        reg_lambda = reg_params.get('lmb')  # a hyper parameter for the EWC regularizer
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if weight_decay != 0 and self.custom_L2 is None:  # CUSTOM L2
...
def train_model(model, criterion, optimizer, lr, dset_loaders, dset_sizes, use_gpu, num_epochs, exp_dir='./',
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if epoch_loss > 1e8 or math.isnan(epoch_loss):
                warnings.warn("ERROR: LOSS WAS TOO HIGH:{}".format(epoch_loss))
                return model, best_acc
            # deep copy the model
            if phase == 'val':
